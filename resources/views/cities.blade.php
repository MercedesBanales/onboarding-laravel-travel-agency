<x-layout>
    <div class="flex w-full justify-end px-4 sm:px-6 lg:px-8 lg:pt-6">
        <button onclick="openCityForm()">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="oklch(0.511 0.262 276.966)" class="size-8">
    ...
    <x-city-table></x-city-table>
</x-layout>
<x-add-city-form></x-add-city-form>

<script>
    const handleSort = () => {
        sortMenu = document.getElementById('sort-menu');
        if (sortMenu.classList.contains('hidden')) sortMenu.classList.remove('hidden')
        else sortMenu.classList.add('hidden');
    }
    ...
    const openCityForm = () => {
        ...
        sideBar.classList.add('opacity-25');
